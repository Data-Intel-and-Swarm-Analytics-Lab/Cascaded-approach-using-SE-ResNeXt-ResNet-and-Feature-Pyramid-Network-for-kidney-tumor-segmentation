...
class ResNet(nn.Module):
    def __init__(self, ResBlock, n_blocks_list=[3, 4, 6, 3],
                 out_channels_list=[64, 128, 256, 512], num_channels=1):
        """
        Args:
...
class ResnetFPNHead(nn.Module):
    def __init__(self, in_channels, num_classes):
        super(ResnetFPNHead, self).__init__()
        # Transposed convolution layers for upsampling
        self.conv1 = nn.ConvTranspose2d(in_channels, 256, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.ConvTranspose2d(256, num_classes, kernel_size=3, stride=1, padding=0)
...
# Create the complete segmentation model
class ResnetFPNModel(nn.Module):
    def __init__(self, resnet_arch='50', num_classes=1):
        super(ResnetFPNModel, self).__init__()
        self.resnet_fpn = ResnetFPN(resnet_arch)
        in_channels = 256  # Adjust based on your FPN configuration
        self.segmentation_head = ResnetFPNHead(in_channels, num_classes)

    def forward(self, x):
        # C2, C3, C4, C5 = self.resnet_fpn(x)
        P2, P3, P4, P5, P6 = self.resnet_fpn(x)
        # print("P4: ",P4.shape)
        segmentation_output = self.segmentation_head(P4)  # Use C4 as input for segmentation head
        return segmentation_output
    

resnet_arch = '50'
num_classes = 1
fpn_model = ResnetFPNModel(resnet_arch, num_classes)
